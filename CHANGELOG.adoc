...
* *playbook-builder*: Allow use of the YAML merge operator in a YAML playbook file (#910)
* *playbook-builder*: Set `process.env` as the default value for the `env` parameter if not specified (#914)
...
* *content-classifier*: Detect case when site start page in playbook contains module name, but not component name
...
* *logger*: Honor `NO_COLOR` environment variable; don't colorize pretty log output if environment variable is set
