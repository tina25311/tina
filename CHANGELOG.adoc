...
* *content-aggregator*: Assign the resolved SHA-1 commit hash for the content source ref (branch or tag) to the `src.origin.refhash` property on the virtual file unless file is taken from worktree (#578)
* *content-aggregator*: Make `refhash` available as template in edit URL pattern (#578)
* *content-classifier*: If `ContentCatalog#resolvePage` cannot locate page, look for alias and dereference if found (#586)
* *content-classifier*: Add `getComponentVersion` method to exported API
* *content-classifier*: Map `asciidoc` property on component to `asciidoc` property on latest version (#543)
* *asciidoc-loader*: Allow target of xref to be a page alias (#586)
* *asciidoc-loader*: Assign value of family-relative path of page to page-src-path attribute on document (#568)
* *asciidoc-loader*: Assign the resolved SHA-1 commit hash for the content source ref (branch or tag) to the `page-origin-refhash` page attribute unless page is taken from worktree (#578)
...
* *content-aggregator*: Use state file to validate repository in cache and reclone the repository if file is missing (i.e., corrupt) (#556)
...
* *content-aggregator*: Brace pattern in start paths value should match explicit entries if wildcard entry is also present (#584)
...
* *content-aggregator*: Ignore dot (hidden) folders when matching start paths unless pattern begins with a dot (#581)
...
* *content-classifier*: Rename `asciidocConfig` property on component version to `asciidoc` (#542)
* *content-classifier*: Change ContentCatalog#resolvePage to delegate directly to ContentCatalog#resolveResource (#597)
...
* *page-composer*: resolvePage helper returns page model instead of virtual file object, which can be disabled using model=false (#541)
* *page-composer*: Assign value of family-relative path of page to srcPath property on page UI model (#568)
* *redirect-producer*: Configure netlify redirects to be forced as recommended by Netlify (#595)
