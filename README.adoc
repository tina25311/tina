...
:img-ci-status: {url-repo}/badges/main/pipeline.svg
...
This project includes a command line interface (CLI) and a default site generator so you can quickly start publishing documentation sites with Antora.
...
We recommend using the latest long term support (LTS) release of Node.js.
While you can use other versions of Node.js, Antora is only tested against {url-node-releases}[active LTS releases].
...
If the command doesn't report an active Node.js LTS version (e.g., v{version-node}), you don't have a suitable version of Node.js installed.
...
Once you've installed nvm, open a *new* terminal and install the latest Node.js LTS release using:
