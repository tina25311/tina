...
The minimum required version of Node is *12.21.0*, as indicated in the `engines.node` key in [.path]_package.json_.
However, we recommend using the Active LTS release (currently Node 16).
These are also the recommended versions of Node for development.

.Why Node 12 and above?
****
