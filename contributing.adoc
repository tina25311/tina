...
If you need to add a new test to the suite, you can use a test in one of the packages as a reference (e.g., [.path]_packages/site-publisher/test/publish-site-test.js_).
...
* Use `deferExceptions` to trap an excepted Error so you can assert it using `.to.throw(message)`.
...
As an alternative, you can run Gulp via `npx`.

 $ npx gulp -v

This saves you from having to install the Gulp CLI globally.
...
If you don't have npm 8, install it into the project using:
 ...
 $ npm i --no-package-lock --no-save npm@8.1
...
RUN npm install -g gulp-cli # install dependencies
