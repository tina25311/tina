...
:url-site-generator-arch: {url-packages}/site-generator-default/devdocs/architecture-guidebook.adoc
:url-site-mapper-arch: {url-packages}/site-mapper/devdocs/architecture-guidebook.adoc
:url-site-publisher-arch: {url-packages}/site-publisher/devdocs/architecture-guidebook.adoc
...
The minimum required version of Node.js is *12.21.0*, as indicated in the `engines.node` key in [.path]_package.json_.
...
Once you've installed nvm, open a new terminal and install Node.js 12 using:

 $ nvm install 12

The above command will install the latest version of Node.js 12.

If you already have other Node.js versions installed, you can configure Node.js 12 as the default for any new terminal.
...
{url-yarn}[Yarn] (command: `yarn`) is the preferred package manager and script runner for the Node.js ecosystem.
...
NOTE: It's a good idea to run `yarn` each time after you run `git pull` to make sure your dependencies are up to date.
Running gulp does not install any updated dependencies.
...
When you ran `yarn` to install the dependencies, it set up a bin stub from [.path]_node_modules/.bin/antora_ to [.path]_packages/cli/bin/antora_.
...
If you install packages globally using the `npm i -g` command, append `@testing` to the name of each package:
 ...
 $ npm i -g @antora/cli@testing @antora/site-generator-default@testing
...
----
"@antora/cli": "testing",
"@antora/site-generator-default": "testing",
----
