...
After {uri-docker-install}[installing Docker], copy the Dockerfile below in a new folder (to limit the build context). The Dockerfile uses the most recent version of Ubuntu.

NOTE: The instructions below are based on linux and have not been tested on othe platforms. It assumes your user id is `1000` (check from command line using `id -u`). If not the `Dockerfile` below should be adjusted.
...
==== Developing from a host machine

If editing code from a host machine and wanting to run tests within the container it is useful to share the code between the container and the host.
...
[TIP]
====
If using `gulp test:watch` make sure there are enough file watches allowed on the system. To adjust this on linux (temporaily), use the following command on the host machine:

 $ sudo sysctl fs.inotify.max_user_watches=524288
====

From here on in, only the command in step 3 above needs to be used from the containing folder to restart the development environment.
