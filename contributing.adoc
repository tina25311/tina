...
The main feature of ES2017 this project depends on is the {url-async-func}[Async Function] (which introduced the `async` and `await` keywords).
...
You can read more about the decision to set Node 8 as the minimum required version in {url-adr-0001}[ADR 0001: Minimum Node Version].
****

If you don't yet have Node installed, or the version of Node you have isn't Node 8 or 10, we strongly recommend using {url-nvm}[nvm] (Node Version Manager) to manage your Node installations.
...
To use Lerna, you need to install it globally using:
...
The CI pipeline is run in the https://docs.gitlab.com/ce/ci/[GitLab CI] environment using the https://store.docker.com/images/node[node:10] docker image.
