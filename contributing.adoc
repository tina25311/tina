...
After fixing your critical bugs or implementing your wonderful new feature, you'll probably want to try out your local antora version on your projects.
The code in [.path]_packages_ is already set up to be run from [.path]_packages/cli/bin/antora_.
When you ran `yarn` to install the dependencies, an alias was set up from [.path]_node_modules/.bin/antora_ to [.path]_packages/cli/bin/antora_.

Therefore, the most convenient way to run your development code is:

* In your antora working directory, run `./node_modules/.bin/antora path/to/the/antora-playbook.yml`

TIP: It might be convenient to put `export ANTORA_DEV=~/path/to/dev/dir/node_modules/.bin/antora` in your .profile or similar file, allowing `$ANTORA_DEV antora-playbook.yml` to work anywhere.
