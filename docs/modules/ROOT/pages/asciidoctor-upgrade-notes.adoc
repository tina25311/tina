...
|The class `page unresolved` in now added to the output when the target of an xref is an invalid resource ID.
Previously, this was only added when the target of an xref was a valid resource ID that couldn't be resolved.
|No action unless you're using a postprocessor that looks for this class in the output.
