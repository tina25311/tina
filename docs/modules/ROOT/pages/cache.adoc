...
The xref:playbook:runtime-cache-directory.adoc#default-cache[default cache directory] varies by operating system.
...
* the xref:playbook:runtime-cache-directory.adoc[cache_dir key] in the playbook.
...
It will also force Antora to download a fresh copy of the UI bundle, if the URL is remote and the xref:playbook:ui-url.adoc#snapshot[snapshot key] is `true`.
