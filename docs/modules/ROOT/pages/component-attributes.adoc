...
You can apply AsciiDoc attributes to all of the pages in a component version by declaring them in its [.path]_antora.yml_ file.

== Component version attributes

AsciiDoc document attributes that are defined in a component version descriptor are referred to as [.term]*component version attributes*.
These document attributes can be built-in AsciiDoc attributes, user-defined AsciiDoc attributes, and user-defined or built-in `page-` prefixed attributes that Antora will pass to the reference UI.
