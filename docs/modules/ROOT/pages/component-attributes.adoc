...
Any xref:playbook:asciidoc-attributes.adoc[site attribute] or component version attribute already set can be referenced from the value of another component version attribute using the attribute reference syntax (e.g., `+You can find the {site-title} at {site-url}.+`). 
To prevent this substitution, insert a backslash in front of the reference (e.g., `\{not-substituted}`).
