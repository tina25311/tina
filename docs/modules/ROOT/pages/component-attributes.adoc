...
You can assign AsciiDoc document attributes for all pages in a component version by declaring the attribute in the [.path]_antora.yml_ file for that component version.
...
The values of user-defined page attributes, such as `page-level`, as well as built-in page attributes can be displayed in a page using the AsciiDoc attribute reference syntax (e.g., `+{page-level}+` or `+{page-version}+`).
...
Check out the xref:page:intrinsic-attributes.adoc#page-attributes[intrinsic attribute reference] for a complete list of the page attributes Antora automatically declares.
