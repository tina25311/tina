...
A component name represents the name of your project, library, or service (e.g., `fauna`, `rudder`, `nrepl`).
...
TIP: If you aren't familiar with how Antora sorts component versions, see xref:how-component-versions-are-sorted.adoc[] before committing to a versioning scheme.

A version, such as a named identifier (e.g., `main`, `jesse`, `edge`) or semantic identifier (e.g., `1.5`, `8`), is assigned to the `version` key in [.path]_antora.yml_.

The `version` key is required.
Its value is used:

* as the version coordinate in page and resource IDs
* as the version segment in page URLs, unless assigned the xref:component-with-no-version.adoc[reserved value master] or the xref:playbook:urls-latest-version-segment.adoc[latest_version_segment playbook key] is set and the component version is the xref:how-component-versions-are-sorted.adoc#latest-version[latest component version]
* to xref:how-component-versions-are-sorted.adoc[sort component versions] in the reference UI
* for display purposes in the reference UI's xref:navigation:index.adoc#component-dropdown[component version selector] and xref:navigation:index.adoc#page-dropdown[page version selector], unless the optional `display_version` key is set
...
xref:component-with-no-version.adoc[Unversioned component versions] can be defined using the reserved value `master`.
