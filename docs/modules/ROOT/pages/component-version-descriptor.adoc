...
. The [.path]_antora.yml_ file at a content source root provides the required and optional component version metadata that Antora assigns to the source files it collects for that component version (located under the [.path]_modules_ directory).
