...
. The presence of an [.path]_antora.yml_ file at a content source root signals to Antora that it should find a sibling [.path]_modules_ directory that contains the source files of a component version.
. The [.path]_antora.yml_ file at a content source root provides the required and optional component version metadata that Antora assigns to the source files it collects from the sibling [.path]_modules_ directory.

[#validate-content-source-root-with-antora-yml]
...
When it finds [.path]_antora.yml_, it searches for a  xref:module-directories.adoc[sibling _modules_ directory] and considers all of the [.path]_modules_ directory's xref:standard-directories.adoc[subdirectories and files] to be part of a xref:component-version.adoc[component version].
...
This minimizes the chances of Antora using server resources in an attempt to process source files that aren't part of a documentation site.
