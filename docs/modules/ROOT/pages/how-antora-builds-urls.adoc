...
A [.term]*site URL* represents the domain or root of your published site.
...
If you're publishing your site to a subfolder, the site domain is followed by a site subpath.
...
A root-relative site URL begins with a forward slash (`/`) and can contain a <<subpath-def,subpath>>.
...
The site pathname represents an internal property, `site.path`, that Antora uses when a publishing feature requires domain-relative URLs.
...
Antora normalizes URLs across the application.
...
If a non-empty value is assigned to the site pathname, Antora prepends the site pathname with a forward slash (`/site-pathname`) and then appends a resource URL.
When the site pathname is empty, a resource URL is already a domain-relative URL.
...
Antora computes absolute URLs when generating sitemaps and assigning the canonical link tag in the head of each page.
...
When the resource belongs to a `ROOT` module, the module segment is dropped from the URL.
...
The resource segment is derived from the family-relative path of the resource's source file.
