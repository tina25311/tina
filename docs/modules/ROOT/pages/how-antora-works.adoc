...
This allows the files to be decoupled from the repository in which they live and makes Antora's URL agnostic xref:asciidoc:page-to-page-xref.adoc[cross references] possible.
+
...
+
A xref:component-version.adoc[component version] typically represents a discreet and unique version of the documentation for a project, library, service, or training course.
