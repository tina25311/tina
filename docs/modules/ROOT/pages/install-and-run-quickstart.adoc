...
If *you have Node.js installed, but it isn't an active LTS version*, you need to upgrade Node.
...
If the installation was successful, the command should report the latest, stable version of Antora for both the CLI and default site generator packages (where `.x` is a placeholder for the latest patch version).
