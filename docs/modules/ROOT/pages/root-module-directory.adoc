...
A [.path]_ROOT_ module directory must contain at least one xref:family-directories.adoc[family directory] with at least one source file.
...
It's recommended that you create a [.path]_ROOT_ module directory because Antora applies special preset behavior to the publishable resources in a <<root-module,ROOT module>>.
