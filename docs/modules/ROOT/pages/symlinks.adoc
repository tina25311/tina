...
A [.term]_symlink_, short for symbolic link, is a shortcut to a file that itself acts like a file.
...
When Antora encounters a symlink to a file, it does not attempt to preserve it as a symlink.
Rather, it creates a regular virtual file, just like it would for any other file it encounters.
...
* The target of the symlink must exist (i.e., it cannot be dangling).
If Antora cannot resolve the symlink, it will throw an error.
* A symlink cannot point to itself.
If Antora detects this scenario, it will throw an error.
* A symlink in a git repository cannot point to a location outside the git repository.
* A symlink in a git repository cannot point to a location in another reference in the git repository.
...
One thing to watch out for is to not map a lot of files into the Antora hierarchy that aren't used in the documentation.
Doing so adds extra processing for Antora that can slow down your build.
Be as surgical and specific as you can about which files you map into the Antora hierarchy.
