...
Issue {url-issues}/930[#970^]:: Drop redundant trailing slash for splat alias of ROOT component with empty version segment.
Set pub URL for splat alias of ROOT component with empty version segment to `/` instead of `/.`.
Issue {url-issues}/931[#971^]:: Fix race condition that was causing the `contextClosed` event to always be emitted before the `sitePublished` event.
