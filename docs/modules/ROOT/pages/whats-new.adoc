...
_**Release date:** 2021.08.11 | *Issue label:* {url-milestone-3-0-0}[3.0.0^]_
...
Issue {url-issues}/799[#799^]:: Introduce an event-based extension mechanism that notifies listeners registered by extensions of key events, also providing access to in-scope pipeline variables.
+
* Add `pipeline` category to the playbook schema to configure the pipeline of the generator.
* Add `extensions` key to the `pipeline` category to specify extensions that listen for pipeline events.
Issue {url-issues}/810[#810^]:: Map repeatable CLI option named `--extension` to add an entry to (or enable an existing one in) the `pipeline.extensions` key in the playbook.
Don't register pipeline extension if extension configuration has a key named `enabled` and its value is `false`.
...
Issue {url-issues}/802[#802^]:: Integrate @antora/user-require-helper to require Asciidoctor extensions, custom providers, user scripts, or a custom generator.
