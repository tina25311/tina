...
Per the normal supported platforms policy, support for Node 8 will be dropped, and likely Node 10 as well.
...
The .adoc extension will no longer be appended to the page or alias ID, if missing (e.g., `component::page`).
This makes way for using the xref facility for non-AsciiDoc pages.
You should begin adding the .adoc extension to all your page and IDs.
