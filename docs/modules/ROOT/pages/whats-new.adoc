...
_**Release date:** 2021.12.22 | *Issue label:* {url-milestone-3-0-0}[3.0.0^]_
...
Project:: Mark deprecations and schedule removals more clearly in source code.
...
* The xref:playbook:runtime-log-destination.adoc[runtime.log.destination category key] in the playbook supports writing log messages to a file or standard stream, with additional settings for buffer size, sync, and append.
...
Antora 3 now honors the environment variables `http_proxy`, `https_proxy`, and `no_proxy`.
...
* Referencing attachments with the link macro (`+link:[]+`) and the `attachmentsdir` attribute are deprecated; use the xref:page:attachments.adoc[AsciiDoc xref macro and the resource ID of the attachment] to link to an attachment instead.
...
= Issues resolved in Antora 3.0.0-beta.x
...
* Report path of a file correctly if it can't be read.
...
Project:: Switch to npm-based build for project.
...
Project:: Change root package name to antora-build and update release script so npm can run version script during release.
...
CLI:: Remove deprecated `--google-analytics-key` CLI option; superseded by `--key`.
