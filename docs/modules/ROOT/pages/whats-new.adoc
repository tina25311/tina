...
:url-milestone-3-0-3: {url-issues}?scope=all&state=closed&label_name%5B%5D=%5BVersion%5D%203.0.3
:url-milestone-3-0-2: {url-issues}?scope=all&state=closed&label_name%5B%5D=%5BVersion%5D%203.0.2
:url-milestone-3-0-1: {url-issues}?scope=all&state=closed&label_name%5B%5D=%5BVersion%5D%203.0.1
...
_**Release date:** 2022.07.19 | *Issue label:* {url-milestone-3-0-3}[3.0.3^]_

== Resolved issues

=== Changed

Issue {url-issues}/977[#977^]:: Defer playbook freezing until after `playbookBuilt` event is fired.

=== Fixed

Issue {url-issues}/942[#942^]:: Use `RedirectMatch` directive for exact directory redirects in httpd redirect file to avoid infinite redirect loop.
Issue {url-issues}/973[#973^]:: Patch match patterns for refname, version, and start paths to work around parsing bug in picomatch.
Issue {url-issues}/974[#974^]:: Keep semantic versions in descending order regardless of prerelease status.
Issue {url-issues}/975[#975^]:: Clone stream on virtual file so complete contents of file are written to each destination.

IMPORTANT: *Antora 2.3 is EOL*.
If you're still using Antora 2, review the checklist on xref:install:upgrade-antora.adoc[] to learn about what changes you should make to your source content and Antora configuration prior to upgrading to Antora 3.

= Antora {page-component-version}.2

_**Release date:** 2022.07.07 | *Issue label:* {url-milestone-3-0-2}[3.0.2^]_

== Resolved issues

=== Fixed

Issue {url-issues}/970[#970^]:: Drop redundant trailing slash for splat alias of ROOT component with empty version segment.
Set pub URL for splat alias of ROOT component with empty version segment to `/` instead of `/.`.
Issue {url-issues}/971[#971^]:: Fix race condition that was causing the `contextClosed` event to always be emitted before the `sitePublished` event.
This affected users who used extensions that relied on these events or a script that uses the `generateSite(args, env)` API from the site generator.
CLI:: Ignore `COLUMNS` environment variable if it contains a non-integer value

=== Maintenance

* Update CI matrix to verify that Antora works on Node.js 18.

= Antora {page-component-version}.1

_**Release date:** 2022.01.13 | *Issue label:* {url-milestone-3-0-1}[3.0.1^]_

== Resolved issues

=== Changed

Issue {url-issues}/930[#930^]:: Make `env` property on playbook enumerable, but don't freeze the value.
Issue {url-issues}/931[#931^]:: Include family (UI) and path in message of duplicate UI file error.
Logger:: Add direct support for `FORCE_COLORS=1` environment variable to enable colors in prettified log message.

=== Fixed

Issue {url-issues}/929[#929^]:: Allow email address as username in content source URL defined in playbook.
Issue {url-issues}/935[#935^]:: Don't escape special regex characters in target URL of nginx rewrite rule.
...
The Antora CLI and site generator are now bundled together as a package named *antora*.
...
Extensions can be registered and configured using the `antora.extensions` key in the playbook.
...
All Antora and Asciidoctor messages and errors, except for CLI warnings, are routed through the logger by default.
...
* The xref:playbook:runtime-log-format.adoc#level-format-key[log.level_format key] allows the log level format of JSON messages to be reported as numbers instead of labels.
* The xref:playbook:runtime-log-destination.adoc[log.destination category key] supports writing log messages to a file or standard stream (`file` key) and controls how the messages are written (`buffer_size`, `sync`, and `append`).
...
Antora 3 now depends on the latest patch version of Asciidoctor.js 2.2, which provides Asciidoctor 2.0.x.
...
These files are configured using the xref:playbook:ui-supplemental-files.adoc#supplemental-files-key[ui.supplemental_files key] in the playbook.
...
We're declaring this feature as stable in Antora 3 and have included xref:playbook:ui-supplemental-files.adoc[documentation] for it.
