...
You do not need to set the `toc::[]` macro on each page for the widget to appear.
...
Daniel Mulholland ({url-gitlab}/danyill[@danyill^]):: For adding documentation about https://gitlab.com/antora/antora/-/blob/main/contributing.adoc#user-content-develop-with-docker[developing Antora with Docker] and Gulp test errors to the contributing guide.
{url-issues}/388[#388^]
...
[#deprecation]
== Deprecations scheduled for removal in Antora 3.0
