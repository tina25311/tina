...
Let's break down the AsciiDoc syntax and resource ID coordinates you need to include an example into a page.
...
The brackets can contain an optional list of attributes such as `indent`, `lines`, or `tags`.
...
NOTE: Antora supports filtering the lines of an example file by either line numbers using the `lines` attribute or tags using the `tag` or `tags` attributes.
Filtering by line numbers takes precedence.
See https://asciidoctor.org/docs/user-manual/#include-partial[the Asciidoctor documentation] for full details of the `lines`, `tag`, and `tags` syntax.
