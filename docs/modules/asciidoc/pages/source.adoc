...
.Assign source and an element ID to a block
[#ex-source-id]
------
[source#element-id,name-of-language] <.> <.>
----
Content of source block
  Content of source block
----
------
<.> In the attribute list, assign `source` to the first position followed by an element ID.
An element ID is directly appended to `source` using the hash symbol (`+#+`) and the ID name you define.
...
In <<ex-diff>>, `source` is assigned directly to the first position in the attribute list and an element ID is appended to it with a `+#+`.
...
The names of the languages that are currently supported are listed in the https://gitlab.com/antora/antora-ui-default/-/blob/HEAD/src/js/vendor/highlight.bundle.js[highlight.bundle.js file in Antora's UI repository^].
