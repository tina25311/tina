...
|--clean
|Remove the xref:playbook:playbook-schema.adoc#output-clean-key[output directory] before publishing the site.
|Boolean
|false
...
Overrides the xref:playbook:playbook-schema.adoc#ui-output-dir-key[output dir value] specified in the playbook.
|String
|[.path]_build/site_
