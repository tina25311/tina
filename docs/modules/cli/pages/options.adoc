...
`fatal`
`silent`
|See xref:playbook:environment-variables.adoc#failure-level[ANTORA_LOG_FAILURE_LEVEL variable].
...
|`structured` if CI=true or +
`pretty` in all other environments
a|[%hardbreaks]
`pretty`
`structured`
`json`
|See xref:playbook:environment-variables.adoc#log-format[ANTORA_LOG_FORMAT variable].
...
`fatal`
`silent`
|See xref:playbook:environment-variables.adoc#log-level[ANTORA_LOG_LEVEL variable].
