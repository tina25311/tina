...
You should use {url-object-destructuring}[object destructring^] to pick individual variables out of this object (e.g., `{ playbook }`).
...
To retrieve multiple variables, you would separate the variable names using commas (e.g., `{ playbook, siteCatalog }`).

In addition to the built-in pipeline variables, your extension can also access pipeline variables documented and published by other extensions.
