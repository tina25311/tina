...
To guarantee our timer starts before listeners from other extensions are called, we want our `playbookBuilt` listener to be called before other listeners of the `playbookBuilt` event and we want our `sitePublished` listener to be called after other listeners of the `sitePublished` event.
...
In order to do something more interesting than time execution time and print messages to the console, we need to xref:pipeline/access-pipeline-variables.adoc[access variables in the pipeline].
