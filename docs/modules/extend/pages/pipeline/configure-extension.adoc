...
An extension that you write, or an extension that you use, may require additional settings to configure its behavior.
While the extension could tap into information already available in either the playbook or the content sources Antora is building, it may be necessary to configure the extension directly.
...
These properties are accessible via a pipeline variable named `config` that is passed to the extension's `register` function.
...
To make room for the additional keys, we've converted the entry from a single (string) value to a map.
The require request shifts into the `require` key.
...
The `config` object for the extension is accessed using object dereferencing just like any other pipeline variable.
Thanks to variable scoping in JavaScript, we can access that variable in our listener for the `beforePublish` event.
We use it to build the contents of the [.path]_humans.txt_ file, then add it to the site catalog.
Antora will then include that file in the published site.
...
To help bridge the naming convention mismatch between YAML and JavaScript, Antora automatically transforms snake_case key names into camelCase properties on the configuration object (e.g., `cache_dir` becomes `cacheDir`).
...
Any keys that are inside the `data` key (at any depth) are passed through without being modified.

Let's assume that we want to be able to specify structured content for our [.path]_humans.txt_ file.
We don't really need Antora messing with this structured content, so we can store it inside a key named `data`.
