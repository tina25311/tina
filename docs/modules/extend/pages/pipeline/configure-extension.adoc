...
These properties are accessible via a pipeline variable named `config` that's passed to the extension's `register` function.
...
The `config` object for the extension is accessed using object destructuring just like any other pipeline variable.
Thanks to variable scoping in JavaScript, we can access that variable in our listener for the `beforePublish` event.
We use it to build the contents of the [.path]_humans.txt_ file and add it to the site catalog.
...
To help bridge the naming convention mismatch between YAML and JavaScript, Antora automatically transforms snake_case key names into camelCase properties on the configuration object.
