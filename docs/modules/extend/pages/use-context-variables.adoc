...
Context variables can also be retrieved directly from the generator context using the `getVars` method:

[,js]
----
const { siteCatalog } = this.getVars()
----
...
But we can create a new playbook, change it's properties, and pass our copy back to the context.
...
In <<ex-proxy>>, we will listen for the `contentClassified` event, retrieve the `contentCatalog` context variable, and replace the variable with a proxy of the object.
...
<<ex-proxy>> gives you the starting point to replace the `registerPageAlias` function with your own implementation.
