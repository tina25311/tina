...
== Upgrade checklist
// tag::new[]
Before you upgrade from Antora 2.3 to Antora {page-component-version}, you may need to make some of the following changes to your documentation system.
...
{page-component-version} supports Node.js 14 LTS and 16 LTS.
{page-component-version} doesn't support Node.js LTS versions that have reached end of life (EOL).
...
{page-component-version} dropped support for Asciidoctor.js 1.5.9 (which provides Asciidoctor 1.5.8) and now depends on the latest patch version of Asciidoctor.js 2.2 (which provides Asciidoctor 2.0.x).
...
See xref:ROOT:asciidoctor-upgrade-notes.adoc[] to learn about the affected features and the suggested actions you should take before upgrading to {page-component-version}.

Are you linking to attachment files?::
Using the AsciiDoc link macro (`+link:[]+`) and `+{attachmentsdir}+` attribute reference in the target value to reference an attachment has been deprecated in {page-component-version}.
...
In {page-component-version}, assign the tilde (`~`) to the `version` key in a component version's [.path]_antora.yml_ file to specify it as unversioned.
...
In {page-component-version}, antora-lunr has been replaced by the https://gitlab.com/antora/antora-lunr-extension[Antora Lunr extension] (package: `@antora/lunr-extension`), which is an official Antora project.
This extension uses the new extension facility in {page-component-version} to integrate Lunr into your site without having to resort to using a custom site generator.
We encourage you to migrate from antora-lunr to the Antora Lunr Extension when upgrading to {page-component-version}.

Do you use or maintain a custom site generator?::
In {page-component-version}, the site generator must declare the `@antora/logger` package as a dependency.
...
ifeval::["{route}" == "New"]
See xref:install:upgrade-antora.adoc[Upgrade Antora] for instructions when you're ready to upgrade to Antora {page-component-version}.
