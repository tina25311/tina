...
Some built-in attributes are intended to be set, while others are read-only.
You can also define custom attributes that work in a similar way.

This page introduces these two catagories of attributes, explains how attributes are defined, and refers you to pages where you can find the reference of built-in attributes.

== Built-in attributes

[.term]*Built-in attributes* are xref:asciidoc-attributes.adoc[AsciiDoc document attributes] that either pass information to the page or provide a way for the author to toggle or configure behavior.
...
[.term]*Custom attributes* are xref:asciidoc-attributes.adoc[AsciiDoc document attributes] defined by the author.
You may be familiar with using custom attributes in AsciiDoc to store reusable content, such as a URL or product name.
The value of a custom attribute can then be used within the page (or across pages, depending on where they are defined) using an attribute reference.
...
A custom page attribute allows the author to pass additional information about a page to the UI templates (so not just in the page itself).
