...
Only the xref:resource-id-coordinates.adoc#id-resource[file coordinate] of the xref:resource-id-coordinates.adoc#target[target resource] needs to be entered in the resource ID when the xref:resource-id-coordinates.adoc#current[current page] and target image belong to the same component version and module.
