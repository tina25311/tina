...
At a minimum, an include directive consists of the directive's prefix (`+include::+`), the resource ID of the target page, and a set of square brackets (`[]`).
...
For this exercise, we've opened the file [.path]_ranges.adoc_.
[.path]_ranges.adoc_ is the current page and belongs to the _ROOT_ module of the component version _colorado 5.2_ shown in <<ex-co>>.
+
...
. Let's reference the target page, [.path]_cottonwood-pass.adoc_, from the current page, [.path]_ranges.adoc_, using an include directive.
Select the line in the current page where you want the page's content to be inserted.
...
As shown in <<ex-co>>, both [.path]_cottonwood-pass.adoc_ and [.path]_ranges.adoc_ belong to the same component version and module.
Therefore, only the file coordinate of the target page needs to be assigned as the value of the include directive.
+
--
.ranges.adoc (current page)
[listing,subs=+quotes]
...
You can use the `page$` family coordinate in the resource ID if you prefer, but it isn't required when including a page.
...
That's it!
You've finished creating an include directive that will insert the target page into the current page.
...
.File coordinate in include directive when target and current pages are stored in the same subdirectory
...
CAUTION: You may find that the include directive resolves the target page without specifying the `./` token in its file coordinate.
Any scenarios where relative includes work without the `./` token are deprecated in Antora 3 and won't work in Antora 4.
We strongly recommend that you use the `./` token when referencing a target page that is stored in the same subdirectory as the current page.
...
.current-page.adoc
[listing#ex-module-component-base]
----
include::module:file-coordinate-of-target-page.adoc[optional attribute] <.>

include::version@component:module:file-coordinate-of-target-page.adoc[optional attribute] <.>

include::component:module:file-coordinate-of-target-page.adoc[optional attribute] <.>
----
<.> If the target page and current page belong to the same component version, but don't belong to the same module, the include directive is assigned the module and file coordinates of the target page.
<.> The include directive is assigned the version, component, module, and file coordinates of the target page when the target page and current page don't belong to the same component version.
...
include::cli-options.adoc[tag=compass] <.>

A line of content.
include::addendum.adoc[] <.>
