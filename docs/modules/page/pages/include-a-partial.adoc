...
== AsciiDoc include directive and partials

An AsciiDoc include directive macro inserts content from a partial's source file into another partial or a page.
...
[.path]_ranges.adoc_ is the current page and belongs to the component version _colorado 5.2_ and module _ROOT_, shown in <<ex-co>>.
+
...
. Let's reference the target partial, [.path]_treeline-warning.adoc_, from the current page, [.path]_ranges.adoc_, using an include directive.
Select the line in the current page where you want the partial's content to be inserted.
...
. Enter the resource ID of the target partial.
As shown in <<ex-co>>, both [.path]_treeline-warning.adoc_ and [.path]_ranges.adoc_ belong to the same component version and module.
Therefore, only the `partial$` family coordinate and file coordinate of the target partial needs to be assigned as the value of the include directive.
+
--
.ranges.adoc (current page)
[listing,subs="+quotes"]
...
That's it!
You've finished creating an include directive that will insert the target partial into the current page.
...
<.> File coordinate of the target partial when the target partial and current page are stored in a subdirectory with parallel family-relative directory paths.
...
----
\include::path/to/target-partial-filename.adoc[optional attribute]
----
...
----
<.> The include directive is assigned the version, component, module, family, and file coordinates of the target partial when the target partial and current page don't belong to the same component version.
...
[#latest-version]
=== Link to the latest version of a partial
