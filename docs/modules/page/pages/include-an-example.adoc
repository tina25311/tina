...
== AsciiDoc include directive and examples

An AsciiDoc include directive macro inserts content from an example's source file into a partial or a page.
...
You can specify one or more optional attributes within the square brackets.
...
For this exercise, we've opened the file [.path]_phases.adoc_.
[.path]_phases.adoc_ is the current page and belongs to the component version _mapper 8.0_ and module _testing_, shown in <<ex-map>>.
+
...
. Let's reference the target example, [.path]_timer.js_, from the current page, [.path]_phases.adoc_, using an include directive.
Select the line in the current page where you want the example's content to be inserted.
...
. Enter the resource ID of the target example.
As shown in <<ex-map>>, both [.path]_timer.js_ and [.path]_phases.adoc_ belong to the same component version and module.
Therefore, only the `example$` family coordinate and file coordinate of the target example needs to be assigned as the value of the include directive.
+
--
.phases.adoc (current page)
[listing,subs="+quotes"]
...
That's it!
You've finished creating an include directive that will insert the target example into the current page.
...
<.> File coordinate of the target example when the target example and current page are stored in a subdirectory with parallel family-relative directory paths.
...
----
\include::path/to/target-example-filename.ext[optional attribute]
----
...
----
<.> The include directive is assigned the version, component, module, family, and file coordinates of the target example when the target example and current page don't belong to the same component version.
...
[#latest-version]
=== Link to the latest version of an example
...
Because a version coordinate isn't specified in <<ex-across-components-latest>>, Antora will complete the resource ID assigned to the include directive using the version coordinate -- `1.5` -- from the latest _mapper_ component.
