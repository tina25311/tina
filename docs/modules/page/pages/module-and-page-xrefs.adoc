...
Only the xref:resource-id-coordinates.adoc#id-resource[file coordinate] of the target page needs to be entered in the resource ID when the xref:page-id.adoc#target[current page and target page] belong to the same component version and module.
...
CAUTION: When the resource ID of the target page is appended with a xref:page-id.adoc#id-fragment[fragment] (aka a deep link) and no link text is specified between the square brackets, like `\xref:page.adoc#fragment[]`, the link text isn't automatically populated.
