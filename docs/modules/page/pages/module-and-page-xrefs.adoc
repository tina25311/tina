...
When an xref doesn't specify any link text, Antora uses the target page's `xreftext` value as the link text.
...
A page's title is assigned to the built-in AsciiDoc `xreftext` attribute by default.
You can also set the `xreftext` attribute in a page's header and assign it custom text.
