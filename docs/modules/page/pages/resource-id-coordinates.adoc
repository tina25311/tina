...
If the target resource is located in a subdirectory of a family directory, the file coordinate must specify the family-relative path to the target resource.
...
When the target resource and current page are both located in the same subdirectory of a family directory, the relative path token, `./`, can be used to abbreviate family-relative path segment of the target resource's file coordinate.
