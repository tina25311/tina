...
In this example, both [.path]_modes.adoc_ and [.path]_satellites.adoc_ belong to the same component version and module and [.path]_modes.adoc_ isn't stored in a subdirectory of the [.path]_pages_ folder.
...
That's it!
You've finished creating a cross reference to the target page ([.path]_modes.adoc_) from the current page ([.path]_satellites.adoc_) using the AsciiDoc xref macro.
