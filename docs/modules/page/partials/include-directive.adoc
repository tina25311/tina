...
The steps below assume that the target {family} and current page belong to the same component version and module, so only the xref:resource-id.adoc#id-family[family ({coordinate})] and xref:resource-id.adoc#id-resource[resource] coordinates of the xref:resource-id.adoc#target-resource[target {family} resource] are entered in the resource ID.
...
If you forget to use the `{coordinate}` coordinate, Antora will report an error because it won't be able to find your {family}.
--
