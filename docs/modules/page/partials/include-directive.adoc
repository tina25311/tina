...
When including a page (i.e., a file in the page family), the family segment is optional.
In all other cases, the resource ID must specify the `{coordinate}` family coordinate since the include directive assumes the family is "`page`" if not specified.
+
