...
If the node module name contains a dot, a slash needs to be appended (for example `highlight.js` becomes `highlight.js/`).
If the slash is missing for a node module that contains a dot, Antora will report `Error: Cannot find module`.
