...
[#nav-extensions]
=== Extensions for nav files

Since nav files can contain only list content, most asciidoctor extensions will be useless on nav files.
However, it is possible to write extensions that generate lists, for instance indexes of a module or component.
To register such an extension for use only with nav files, list it under `nav_extensions`.
