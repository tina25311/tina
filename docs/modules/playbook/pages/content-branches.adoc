...
That means Antora will select from the repository the current (for local) or default (for remote) branch (e.g., `main`) as well as any branch that begins with the letter `v` (e.g., `v2.0.x`).
