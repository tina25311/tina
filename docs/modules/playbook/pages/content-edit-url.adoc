...
In the default UI, Antora uses this value (if set) to create an *Edit this Page* link in the upper right corner of each page to links the visitor to the edit interface provided by the hosted git service.
For example, if you click the *Edit this Page* link on this page, your browser will go to GitLab's file editing interface and load the AsciiDoc source for the current page.

The one exception is if the repository is private.
In this case, the default UI does not show the *Edit this Page* link.
You can force the default UI to show this link by setting the `FORCE_SHOW_EDIT_PAGE_LINK` environment variable.
