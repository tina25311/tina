...
The key accepts a URL pattern that contains the URL segments of the git solution or source file view plus several placeholder segments, `+{web_url}+`, `+{refname}+`, `+{reftype}+`, `+{refhash}+`,  `+{start_path}+`, `+{module}+`, `+{family}+`, `+{file_path}+, and `+{path}+`.
...
When `edit_url` is set, either by default or explicitly, Antora computes the value of the `+{web_url}+`, `+{refname}+`, `+{reftype}+`, `+{refhash}+`, `+{start_path}+`, `+{module}+`, `+{family}+`, `+{file_path}+`, and `+{path}+` placeholders for each page according to its content source and file origin information.
...
start_path:: The `+{start_path}+` is the the `start_path` if one is specified inside the playbook.

module:: The `+{module}+` is the name of the module directory of the source file (e.g., ROOT).
 
family:: The `+{family}+` is the name of the module directory of the source file (e.g., pages, partials).

file_path:: The `+{file_path}+` is the path of the source file relative to its family directory. This is either a filename or a relative path if subdirectories are used inside the family directory (e.g., index.adoc, subdirectory/index.adoc).

path:: The `+{path}+` is the path of the source file relative to the root of the repository if the standard Antora directory structure is used. 
It includes the `start_path` if one is specified and is identical to `+{start_path}/modules/{module}/{family}/{file_path}+` or `+modules/{module}/{family}/{file_path}+` if no `start_path` is specified.
