// document which options we use for picomatch
// mention matching across path segments
// brace expression with single entry is ignored {foo}
// globstar??
...
The single or comma-separated string syntax is intented to be used for exact matching only.
...
And a pattern always matches from the beginning to the end of the rename.
...
You can glob all refnames that existing using a lone wildcard:
...
Let's throw away that non-version refname that we inadvertantly matched:
...
Braces (or brace expressions) are patterns that are enclosed in curly braces (`{}`).
...
You can use an alternation expression to match the absense of a character or segmment using an empty entry.
...
IMPORTANT: If the pattern begins with a number followed by a period, and the pattern contains at least one brace expression, the period must be enclose in round brackets (`()`) (e.g., `5(.){7,8}.x`).
This requirement is imposed as a result of a parsing bug in the pattern matching library.
...
By changing the begin value, you can match odd major versions instead:
...
Braces on their own have limited ability to express complex patterns.
To make brace expressions truly powerful, you need to combine them with extended globbing.

== Extended globbing and repetition

So far we've been matching single occurances of segments, whether they be character sequences, alternations, or ranges.
...
This use of repeition is *far* more efficient than the following range, which you should avoid using:
