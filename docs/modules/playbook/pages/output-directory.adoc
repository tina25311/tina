...
If both an `fs` provider `path` and `dir` are specified, the value assigned to `dir` overrides xref:output-filesystem-provider.adoc#path[the first fs provider path].
When the output directory is specified from the CLI using the xref:cli:index.adoc#to-dir[--to-dir <dir> option], it also overrides the `dir` and first `fs` provider `path` in a playbook.
...
The `clean` key can also be activated via the CLI using the xref:cli:index.adoc#clean[--clean option].
