...
Unless the `path` key or xref:output-directory.adoc[dir key] is specified, the default path for the `fs` provider is [.path]_build/site_.
...
When the xref:output-directory.adoc[dir key] is specified, its value is assigned to (or overrides) the first `fs` provider `path`.
When the output directory is specified from the CLI using the xref:cli:index.adoc#to-dir[--to-dir <dir> option], it also overrides the first `fs` provider `path` (as well as `dir`) in a playbook.
...
It can be applied as part of an `fs` provider configuration instead of xref:output-directory.adoc#clean-key[directly under the output key].
...
include::page$output-archive-provider.adoc[tag=multiple]
