...
You can pass these credentials to Antora in several ways, including the built-in git credential store, a custom credential store, or even your own credential manager.

First, let's review the authentication credential types Antora accepts by default and how to set them up.
...
Keep in mind that while some git hosts supports this means of authentication, others do not.
You may have better luck using an access token.

If you have two-factor authentication (2FA) enabled on your account, you most certainly won't be able to use the username/password pair to authenticate with a private repository.
(You'll receive the error _401 HTTP Basic: Access Denied_ when you try).
Thus, if you have two-factor authentication enabled on your account, perhaps even if you don't, you'll need to use an access token.
...
[subs=+quotes]
----
https://**TOKEN**:@github.com:org/project-docs.git
...
content:
  sources:
  - url: https://**TOKEN**:@github.com:org/project-docs.git
