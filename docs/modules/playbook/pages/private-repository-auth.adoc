...
git:
  plugins:
    credentialManager: ./custom-git-credential-manager.js
----
...
In this case, you pass the credential manager to Antora using the `-r` option instead of registering it with the playbook:

 $ antora -r ./custom-git-credential-manager.js antora-playbook.yml

When using a self-registering plugin, if you've installed Antora globally using `npm`, you may run into problems getting your custom credential manager to work.
...
Since self-registering a plugin is more complex, we will proceed with the example of registering the plugin in the playbook.
...
git:
  plugins:
    credentialManager: ./system-git-credential-manager.js
----
