...
You can apply AsciiDoc and `page-` attributes to all of the pages in a site by declaring them in its playbook file.
...
These attributes are applied and available to all of a site's pages according to the attribute precedence rules.

== Set and unset attributes in the playbook

Each site attribute is entered on its own line under the playbook's `asciidoc` and `attributes` category keys.
