...
* The `page.canonicalUrl` is set in the UI model, which gets used by the resference UI to create the canonical link tag in the head.
...
When required, Antora uses the site URL to construct absolute and doamin-relative URLs to pages in your site, which will always include the subpath, if specified.
