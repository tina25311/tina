...
The only required file in the UI bundle is the default layout for pages (e.g., [.path]_layouts/default.hbs_).
...
The filesystem path may be a directory where the UI bundle has been extracted.
...
Here's the path to the location where the UI bundle has been extracted (or perhaps was never a ZIP archive in the first place):
