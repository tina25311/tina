...
The UI files are published to a directory labeled with a single underscore, [.path]_++_++_ , located relative to the site's xref:output-directory.adoc[output directory].
