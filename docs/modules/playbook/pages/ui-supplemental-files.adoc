...
Antora scans this directory for files and adds them to loaded UI bundle.
...
Next, populate it with HTML and optional template logic that you want to include.
...
The contents can be specified directory or sourced from a file on the filesystem.
...
WARNING: If the UI bundle contains a [.path]_ui.yml_ file, you will need to replicate its contents when defining on in the supplemental UI.
