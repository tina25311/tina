...
----
<1> Enter the parent key `ui`, followed by a colon (`:`), then press kbd:[Enter].
<2> Press the space bar twice so the `bundle` key is a child of `ui`.
Enter the key `bundle`, followed by a colon (`:`), then press kbd:[Enter].
<3> The `url` key must be nested under `bundle`.
Enter `url`, followed by a colon and a blank space (`: `), then enter the URL or filesystem path value.

Remember that the UI bundle must be a ZIP archive.

[#remote-bundle]
== Fetch and load a remote bundle
