...
describe('loadAsciiDoc()', () => {
  ...
  describe('include directive', () => {
    ...
    it('should skip include directive if max include depth is 0', () => {
      ...
      expect(messages[0].trim()).to.equal(
        'asciidoctor: ERROR: page-a.adoc: line 1: maximum include depth of 0 exceeded'
      )
    })
  ...
  ;[
    ...
    resolveConfig({
      dir: FIXTURES_DIR,
      asciidoc: {
        converters: ['./converter/delegating-converter.js'],
      },
    }),
    resolveConfig({
      dir: FIXTURES_DIR,
      asciidoc: {
        converters: ['./converter/delegating-converter.js', './converter/delegating-converter.js'],
      },
    }),
  ].forEach((config) =>
    describe(`page reference macro using custom converters ${(config.converters || []).map(
      (converter) => converter(null, null).name
    )}`, () => {
      ...
      it('should process xref inside of a footnote macro', () => {
        ...
        ;[
          ...
          setInputFileContents(contents)
          const doc = loadAsciiDoc(inputFile, contentCatalog, config)
  ...
  describe('resolveConfig()', () => {
    ...
    it('should load specified converters', () => {
      const playbook = {
        dir: FIXTURES_DIR,
        asciidoc: {
          converters: ['./converter/delegating-converter.js', './converter/nondelegating-converter.js'],
        },
      }
      const config = resolveConfig(playbook)
  ...
  describe('converter', () => {
    it('should not convert a page reference with topic and page using nondelegating converter', () => {
      const playbook = {
        dir: FIXTURES_DIR,
        asciidoc: {
          converters: ['./converter/nondelegating-converter.js'],
        },
      }
      const config = resolveConfig(playbook)
      const contentCatalog = mockContentCatalog({
