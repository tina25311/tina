...
To publish a release, first make sure the <<Credentials,npm token>> is still valid and that the end copyright year is current in README.adoc and docs/modules/ROOT/pages/project/copyright-and-license.adoc (unless the end year is already "`present`").
...
The release job delegates to https://lerna.js.org[Lerna] to perform the release.
...
* https://gitlab.com/antora/antora/blob/main/scripts/prepublish.js[prepublish.js]
* https://gitlab.com/antora/antora/blob/main/scripts/postpublish.js[postpublish.js]
...
As part of this dance, we need to patch the version of libnpmpublish that lerna uses so that it includes the `readmeFilename` key in the metadata it uploads.
We think this allows the Yarn registry to find the README file.
That patch is applied by [.path]_scripts/patch-libnpmpublish.js_.
To verify that patch when testing a release, print the metadata built in [.path]_lerna/node_modules/@evocateur/libnpmpublish/publish.js_ to a file.
On the other site of the wire, write the metadata passed to the `afterChange` callback in verdaccio to a file to see that it is included.

In addition to these two package-scope scripts, the https://gitlab.com/antora/antora/blob/main/scripts/postpublish.js[version.js] script runs once at the start of the Lerna lifecycle to apply the following changes before the files are committed and tagged for the release:
...
* Update the end copyright year in all package README files (unless the end year is "`present`").
...
If the value assigned to the RELEASE_VERSION variable matches one of the increment keywords (major, minor, patch, premajor, preminor, prepatch, prerelease), that value is passed to Lerna's version bump argument.
Consult the Lerna documentation for details about how these values determine the next version number.
Otherwise, the value is assumed to be an explicit version number, which is also passed to Lerna's version bump argument (as Lerna figures out which is which).

If the RELEASE_NPM_TAG variable is set, its value is passed to Lerna's `--dist-tag` option.
...
But default, the release is cut from the main branch.
...
CAUTION: If you release from a branch other than main, you'll need to carry over the change that the release scripts makes to the CHANGELOG to the main branch manually in order to retain a continuity of releases.
...
. Rebase the releases branch on top of the release branch (e.g., main).
...
//NOTE: In the future, we may decide to add multiple release branches, one for each version line (e.g., 1.x, 2.x, main, etc.)
